...
module.exports = class extends Generator
  ...
  prompting:->
    @log yosay("Welcome to the wicked #{chalk.red 'coffee-mithril'} generator!")
  ...
  install:->
    ...
    done = => @log chalk.green('ALL DONE! Get Cracking!')
    @yarnInstall().then (yarnError)=>
      if yarnError
        @npmInstall().then (npmError)=>
          if npmError
            @log chalk.red(npmError)
          else done()
      else done()
    return
